...
public class Polinomio {
	...
	/**
	 * Crea un polinomio definado por un String. La representación del polinomio es
	 * una secuencia de monomios separados por '+' (y posiblemente con caracteres
	 * blancos). Un monomio esta compuesto por tres partes, el coefficiente (un
	 * entero), el caracter 'x' (el variable), y el exponente compuesto por un un
	 * caracter '^' seguido por un entero. Se puede omitir multiples partes de un
	 * monomio, ejemplos:
	 * 
	 * <pre>
	 * {@code
	...
	public Polinomio(String polinomio) {
		...
		if (!polinomio.equals("0")) {
			...
			for (String monomioElementos : elementos) {
				int coeficiente;
				int exponente;
				if (monomioElementos.contains("x^")) {
					String[] partes = monomioElementos.split("x\\^");
				...
				} else if (monomioElementos.contains("x")) {
					String coeficienteString = monomioElementos.replace("x", "").trim();
					coeficiente = coeficienteString.isEmpty() ? 1 : Integer.parseInt(coeficienteString);
					exponente = 1;
				} else {
					coeficiente = Integer.parseInt(monomioElementos.trim());
					exponente = 0;
				}
	...
	public static Polinomio resta(Polinomio p1, Polinomio p2) {
		...
		while (cursor2 != null) {
			Monomio monomio2 = cursor2.element();
			resultado.terms.addLast(new Monomio(-monomio2.getCoeficiente(), monomio2.getExponente())); // Negar el
																										// coeficiente
																										// de p2
			cursor2 = p2.terms.next(cursor2);
		}
	...
	public long evaluar(int valor) {
		...
		while (cursor != null) {
			resultado = (int) ((resultado
					+ (Math.pow(valor, cursor.element().getExponente())) * cursor.element().getCoeficiente()));
	...
	/**
	 * Devuelve el exponente (grado) del monomio con el mayor grado dentro del
	 * polinomio Si el polinomio es vacio (la representacion del polinomio "0")
	 * entonces el valor devuelto debe ser -1.
	 * 
