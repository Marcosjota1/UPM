...
void cifrado_cesar(int op, char *texto) {
  ...
  if (op != 0 && op != 1){
    ...
    int ajuste = (op == 0) ? 1 : -1; /*Ajuste para codificación o decodificación*/ 
                                     /*op 0 suma = codifocar / op != 0 resta = decodificar*/
...
int cargar_diccionario(FILE *fich_dicc) {
  ...
  /* cargas variables locales globales */
...
for (i = 0; i < (int)n_palabras && !ferror(fich_dicc) && fgets(pal, LONG_PALABRA_WORDLE + 2, fich_dicc); ++i) {

  /* elimino salto de linea final de la palabra */
  if (pal[strlen(pal) - 1] == '\n')
    pal[strlen(pal) - 1] = '\0';
...
void vaciar_diccionario(void) {
  ...
  for (i = 0; i < (int)n_palabras; i++) {
    free(diccionario[i]);
    diccionario[i] = NULL;  /*Establecer el puntero a NULL después de liberar la memoria*/ 
  }
  free(diccionario);
  diccionario = NULL;  /*Establecer el puntero a NULL después de liberar la memoria*/ 
