        ...
        this.parameterTypes = parameterTypes;
            ...
            public String parse(BufferedWriter parseWriter, List<Token> tokens, List<List<TS>> tablaGlobal2 ) throws IOException {
                ...
                private boolean yaTieneTipo(String id) { //Evitar declaraciones multiples
                    ...
                    if(tipoExistente.equals("entero") || tipoExistente.equals("cadena") || tipoExistente.equals("boolean")){
                        return true;
                    }else{
                        return false;
                    }
                ...
                private void inicializar(String pos) {
                    ...
                    if (tipo == null || tipo.isEmpty()) {
                        // Si estamos en la tabla global
                        if (TSA == 0) {
                            // Asignar tipo "entero" y actualizar el desplazamiento
                            addTypeDesp(pos, "entero", getAncho("entero"));
                        } else { // Estamos en una tabla local
                            try {
                                // Obtener la tabla local correspondiente
                                ...
                                if (simboloLocal == null) {
                                    System.err.println("Error: Símbolo no encontrado en la tabla local en la posición: " + pos);
                                    return;
                                }
                
                                // Eliminar el símbolo de la tabla local
                                tablaLocal.remove(simboloLocal);
                
                                // Actualizar la posición y desplazamiento para la tabla global
                                simboloLocal.despl = DESPG;
                                DESPG += getAncho("entero");
                
                                // Asignar tipo "entero" al símbolo
                                simboloLocal.tipo = "entero";
                
                                // Agregar el símbolo actualizado a la tabla global
                                TablaGlobal.get(0).add(simboloLocal);
                
                                // Confirmación opcional (puedes descomentar si es necesario)
                                // System.out.println("El identificador en la posición " + pos + 
                                //     " se movió de la tabla local a la global y se asignó el tipo 'entero'.");
                            } catch (NumberFormatException e) {
                                System.err.println("Error: La posición '" + pos + "' no es un número válido.");
                            } catch (IndexOutOfBoundsException e) {
                                System.err.println("Error: Índice de TSA fuera de rango: " + TSA);
                            }
                        }
                    }
                }               
