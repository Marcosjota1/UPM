...
if (reservadas.containsKey(word)) {
    ...
    if (currTable > 0) {
        ...
        List<TS> globalTable = tablaGlobal.get(0);
        ...
        if (!tsOptionalLocal.isPresent()) {
            ...
            if (tsOptionalGlobal.isPresent() && currTable > 1) {
                ...
                currentTable.add(nuevoSimbolo); // Ensure the new symbol is added to the current table
                if (capturaParametros) {
                    parametrosTemporales.add(nuevoSimbolo);
                }
                tokens.add(new Token("id", String.valueOf(currentTable.indexOf(nuevoSimbolo)))); // Correctly track the position
            }
        } else {
            tokens.add(new Token("id", String.valueOf(currentTable.indexOf(tsOptionalLocal.get())))); // Use existing symbol position
        }
                    }
                }
            }
