                ...
                if (Character.isLetter(ch)) {
                    ...
                    if (reservadas.containsKey(word)) {
                        ...
                        if (!tsOptionalLocal.isPresent()) {
                            TS nuevoSimbolo = new TS(word, "", 0, "", "", 0, posParam); // Usar contador de parámetros
                            posParam++; // Incrementar el contador de parámetros
                            if (capturaParametros) {
                                parametrosTemporales.add(nuevoSimbolo); // Agregar temporalmente para su uso al procesar '{'
                            } else {
                                currentTable.add(nuevoSimbolo); // Solo añadir a la tabla local
                            }
                            tokens.add(new Token("id", String.valueOf(nuevoSimbolo.pos))); // Usar posición del contador
                        } else {
                        ...
                        //for (int i = 0; i < 5; i++) { //5 por probar, cambiar luego
                            ...
                            for (TS simbolo : tabla) {
                                if (simbolo.pos == posBuscada) {
                                    ...
                                    simbolo.despl = DESPG;
                                    DESPG += ancho;                                    
                                    System.out.println("Tipo actualizado: Tabla " + TSA + ", Posición " + posBuscada + ", Tipo: " + tipo + ", Desp: " + DESPG);
                                    return;
                                }
                            }
