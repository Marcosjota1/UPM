package EjerciciosClase.entregable3;
...
//       System.out.println("No SC");
   ...
   static class Decrementador extends Thread {
      public void run () {
         for (int i = 0; i < N_PASOS; i++) {
            System.out.println("n(dec) = " + n);
            
            // Sección no crítica


            // Protocolo de acceso a la sección crítica
            //Mientras el otro proceso esta en sc, este esta en no_sc y viceversa
            while (en_sc) { no_sc();}
            en_sc = true;
