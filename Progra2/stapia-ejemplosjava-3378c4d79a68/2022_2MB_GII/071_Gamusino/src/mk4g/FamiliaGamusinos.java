...
public class FamiliaGamusinos {
	...
	private static final int porcentajeIndigo = 50;
	...
	public FamiliaGamusinos() {
		for ( int i = 0; i < MAX_GAMUSINOS; ++i ) {
			...
			if ( aleatorio <= porcentajeVioletas ) {
				gamusinos[i] = new GamusinoVioleta(random.nextInt(20));
			} else if ( aleatorio <= porcentajeVioletas + porcentajeVerdes) {
				gamusinos[i] = new GamusinoVerde();
			} else {
				gamusinos[i] = new GamusinoIndigo(random.nextInt(3)+1, random.nextInt(4));
			}
		}
	}
